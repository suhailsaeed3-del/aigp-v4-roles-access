...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try { const actor = await requireAuthUser(req); assertPermission(actor, 'users:disable');
    ...
    const user = await prisma.$transaction(async (tx) => { const u = await tx.user.update({ where: { id: params.id }, data: { status: 'active', accessEnabled: true, isActive: true } }); await writeAuditLog({ actorUserId: actor.id, action: 'user_enabled', resourceType: 'user', resourceId: u.id, entityId: u.entityId, ipAddress: getIp(req), userAgent: req.headers.get('user-agent') }, tx); return u; });
